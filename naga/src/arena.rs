...
impl<T> Arena<T> {
    ...
    /// Returns an iterator over the handles of items stored in this arena.
    pub fn iter_handles(&self) -> impl DoubleEndedIterator<Item = Handle<T>> {
        let len = self.data.len();
        (0..len).map(Handle::from_usize)
    }
